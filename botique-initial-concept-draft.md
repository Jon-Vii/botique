...
**Project**: Autonomous organizations — AI agents that independently run competing Etsy-like shops in a simulated marketplace. Each shop is a minimal autonomous org: a boss agent making strategic decisions, creating products, managing customers, and competing for market share, with delegation or specialist sub-agents as a possible later addition. The platform is informed by Etsy's seller API patterns and data model, making the core seller tooling theoretically portable to a real Etsy-like platform.
...
**Where the thinking power goes**: Not the infrastructure. The platform API, database, frontend — those are solved problems and highly AI-codeable. The hard part is everything surrounding what the agents see and do: the system prompt, the morning briefing format, the tool response formatting, the turn structure, how strategy evaluation works, and whether/how delegation should work at all. This is what hackathon judges will care about — "what is the agent actually doing and why?" If agents are visibly strategizing, adapting, and competing, that's the win.
...
Three major systems, with a CLI bridge layer:
...
└──────────────────────┬──────────────────────────────┘
                       │ WebSocket + REST
┌──────────────────────┴──────────────────────────────┐
│  SYSTEM 1: BOTIQUE PLATFORM + SIMULATION            │
│  FastAPI server implementing Botique's seller API   │
│  plus the non-AI marketplace world behind it:       │
│  - storefront endpoints for listings, shops,        │
│    orders, reviews, messages                        │
│  - customer personas, demand, search, and trends    │
│  - purchase probability and day resolution          │
│  - optional narrative generation for DMs/reviews    │
│                                                     │
│  OPEN DESIGN QUESTIONS:                             │
│  - How do trends shift? Static / seasonal / random? │
│  - How is listing quality scored?                   │
│  - What drives search ranking?                      │
│  - How realistic vs. controllable?                  │
│  - Math for high-freq vs. LLM for low-freq events?  │
└──────────────────────┬──────────────────────────────┘
                       │ REST API calls
┌──────────────────────┴──────────────────────────────┐
│  CLI TOOL LAYER                                     │
│  Python functions wrapping API calls                │
│  Botique-first tool names with internal             │
│  Etsy-informed compatibility mapping                │
...
**Key principle**: System 1 doesn't know its clients are LLMs. System 2 doesn't know the marketplace is simulated. The CLI layer is the bridge. Swapping the CLI layer's backend from simulated Botique to another Etsy-like seller platform should require minimal or no changes to agent code.
